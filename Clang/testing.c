...
int main (int argc, char * argv[]) {
	...
	bbia_bitflag_set (secondTest, 128);
	bbia_print_levelValue (secondTest);

	puts ("After bitflag unset :");
	bbia_bitflag_unset (secondTest, 128);
	bbia_print_levelValue (secondTest);
	...
	for (int lvl = BBIA_LEVEL_TOP; lvl >= 0; lvl--)
		bbia_at_set (firstTest, lvl, (BBIA_BIT_LAST | BBIA_BIT_LAST>>1 ));
	...
	for (int lvl = BBIA_LEVEL_TOP; lvl >= 0; lvl--)
		bbia_at_set (firstTest, lvl, (0x1 | 0x2
	| 0x4 | 0x8 ));
