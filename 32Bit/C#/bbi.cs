...
public class BitBigInt
    {
        ...
        public class Constants
        {
  		...
  		*/
            ...
            public const uint LEVEL_TOP = LEVEL_COUNT - 1;
        ...
        public class Exceptions
        {
            ...
            public class InvalidBitInBitflag : BBIException
            {
                ...
                public InvalidBitInBitflag(uint bit)
        ...
        public class uintArithmetics
        {
            ...
            static public uint Bitflag(uint bit)
            {
            ...
            static public bool OutOfUint_Add(uint to, uint test) => to > uint.MaxValue - test;

            static public bool OutOfUint_Sub(uint from, uint test) => from < uint.MinValue + test;

            static public bool BaseOf2(uint value)
            {
            ...
            static public int Compare(uint a, uint b) => (a == b) ? 0 : (a > b) ? 1 : -1;
        }
        ...
        public bool Check_IsZero() => this.Check_IsInteger(0);
        public bool Check_IsOne() => this.Check_IsInteger(1);
        public bool Check_IsSystemInteger() => (this.lvlButton == Constants.LEVEL_TOP) ? true : false;


        public int CompareUnsigned(BitBigInt b)
        {
            int curCompare = 0;
            uint curLvl = (this.lvlButton <= b.lvlButton) ? this.lvlButton : b.lvlButton;
            for (; curLvl <= Constants.LEVEL_TOP; curLvl++)
            {
                curCompare = uintArithmetics.Compare(at[curLvl], b.at[curLvl]);
                if (curCompare != 0)
                    return curCompare;
            }
            return 0;
        ...
        {
            int res = this.CompareUnsigned(toCompare);
        ...
        public void Bits_ShiftLeft(uint value)
        {
            ...
            uint[] savedBits = new uint[Constants.LEVEL_TOP];
            uint[] bitMask = { 0, 0 };
            ...
            for (uint lvl = Constants.LEVEL_TOP; lvl > this.lvlButton; this.at[lvl] <<= (int)value, lvl--)
            {
                bitMask[1] = this.at[lvl] & bitMask[0];
                bitMask[1] >>= (int)(Constants.INTEGER_SIZE - value);
                savedBits[lvl - 1] = bitMask[1];
            }
            this.at[this.lvlButton] <<= (int)value;
        ...
        public void Bits_ShiftRight(uint value)
        {
            ...
            // we set saved bits
            for (uint lvl = this.lvlButton; lvl < Constants.LEVEL_TOP; this.at[lvl] >>= (int)value, lvl++)
            {
                bitMask[1] = this.at[lvl] & bitMask[0];
                bitMask[1] <<= (int)(Constants.INTEGER_SIZE - value);
                savedBits[lvl] = bitMask[1];
            }
            this.at[Constants.LEVEL_TOP] >>= (int)value;
        ...
        public BitBigInt Bits_Empty() => new BitBigInt();
            ...
            else
            {
                ...
                for (; (level >= this.lvlButton) || (outBit != 0 && level > 0); level--)
                    for (uint bitPos = 1; bitPos <= Constants.INTEGER_SIZE; bitPos++)
                    {
                        ...
                        curBit = ((this.at[level] & uintArithmetics.Bitflag(bitPos)) != 0) ? 1 : 0;
                        // if nBit of from is enabled
                        if ((from.at[level] & uintArithmetics.Bitflag(bitPos)) != 0) curBit++;
        ...
        internal protected void _sub_bbi_op(BitBigInt subtrahend)
        {
            ...
            else
            {
                ...
                for (; level >= this.lvlButton; level--)
                    ...
                    {
                        curBitFirst = ((this.at[level] & uintArithmetics.Bitflag(bitPos)) != 0) ? 1 : 0;
                        curBitSecond = ((subtrahend.at[level] & uintArithmetics.Bitflag(bitPos)) != 0) ? 1 : 0;
        ...
        private void _multiplicationByBitAnd_op(BitBigInt temp)
        {
            ...
            {
                if (lvlBit != 0)
                    curBitVal = ((multiplier.at[lvl] & uintArithmetics.Bitflag(lvlBit)) != 0) ? true : false;
                else
                    curBitVal = ((multiplier.at[lvl--] & uintArithmetics.Bitflag(Constants.INTEGER_SIZE)) != 0) ? true : false;
        ...
        private void _divisionBy_op(BitBigInt division, DIVISION_OR_MOD flag)
        {
            ...
            currentMultiplierOfDivision.Sub(1); // analog for i--
                                                // division result
            if (flag == DIVISION_OR_MOD.DIVISION)
            {
            ...
            // mod result
            else if (flag == DIVISION_OR_MOD.MOD)
            {
                currentDifference.Copy(division);
                currentDifference.Mult_Unsigned(currentMultiplierOfDivision);
        ...
        public void Div(BitBigInt division)
        {
            ...
            this._divisionBy_op(division, DIVISION_OR_MOD.DIVISION);
            this.LvlButton_Configure();
            if (toChangeSign == true) this.Sign_Change();
        }
        ...
        public void Mod(BitBigInt division)
        {
            ...
            this._divisionBy_op(division, DIVISION_OR_MOD.MOD);
            this.LvlButton_Configure();
        }
        ...
        public void Log(uint _base, bool isSigned)
        {
            ...
            if (this.CompareUnsigned(_base) == -1)
            {
                return;
            }
            if (this.Check_IsZero() || _base == 0)
            {
                return;
            }

            uint curPower = 1;
            BitBigInt compare = new BitBigInt(isSigned, _base);
            while (this.CompareUnsigned(compare) == 1)
            {
                compare.Set_SystemInteger(isSigned, _base);
                compare.Pow(++curPower);
            }
            this.Set_SystemInteger((isSigned == true && curPower % 2 != 0) ? true : false, curPower);
        }

        public BitBigInt Log_New(uint _base, bool isSigned)
        {
        ...
        public void Log(BitBigInt _base)
        {
            ...
            if (this.Check_IsZero() || _base.Check_IsZero())
            {
                return;
            }

            uint curPower = 1;
            BitBigInt compare = new BitBigInt(_base);
            ...
            this.Set_SystemInteger((_base.sign == true && curPower % 2 != 0) ? true : false, curPower);
        }

        public BitBigInt Log_New(BitBigInt _base)
        {
            ...
            compare.Set_SystemInteger((_base.sign == true && curPower % 2 != 0) ? true : false, curPower);
            return compare;
        }
